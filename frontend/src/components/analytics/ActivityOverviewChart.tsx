...
export default function ActivityOverviewChart() {
  const legend = [
    { label: 'URLs', color: '#3b82f6' },
    { label: 'Websites', color: '#10b981' },
    { label: 'Extensions', color: '#a855f7' },
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-5 flex flex-col justify-between h-[340px] hover:border-slate-700 transition-colors text-left">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-sm font-bold text-white leading-none">Activity Overview</h2>
          <span className="text-[10px] text-slate-500 block mt-0.5">Scanned objects summary</span>
        </div>
      ...
      <div className="flex-1 w-full min-h-0 text-[8px] mt-2.5">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={analyticsDemoData.activityOverview} margin={{ top: 5, right: 5, left: -32, bottom: -5 }}>
            ...
            />
            <Line type="monotone" dataKey="urls" stroke="#3b82f6" strokeWidth={1.5} dot={false} />
            <Line type="monotone" dataKey="websites" stroke="#10b981" strokeWidth={1.5} dot={false} />
            <Line type="monotone" dataKey="extensions" stroke="#a855f7" strokeWidth={1.5} dot={false} />
            <Line type="monotone" dataKey="behavior" stroke="#f59e0b" strokeWidth={1.5} dot={false} />
          </LineChart>
        </ResponsiveContainer>
      </div>
