...
export default function TopRiskyIpsTable() {
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-5 flex flex-col h-[380px] hover:border-slate-700 transition-colors justify-between text-left overflow-hidden">
      {/* Header */}
      <div className="mb-4">
        <h2 className="text-base font-bold text-white leading-none">Top Risky IP Addresses</h2>
        <span className="text-xs text-slate-500 mt-1 block">Most active malicious IP nodes in telemetry</span>
      </div>
        ...
        <table className="w-full text-left text-xs border-collapse">
          <thead>
            <tr className="border-b border-slate-800 text-slate-500 font-bold uppercase tracking-wider text-[10px]">
              <th className="py-2.5 pb-2">IP Address</th>
              <th className="py-2.5 pb-2 text-center">Risk Score</th>
              <th className="py-2.5 pb-2 text-right">Threats</th>
              <th className="py-2.5 pb-2 text-center w-12">Country</th>
              <th className="py-2.5 pb-2 text-right w-12">Trend</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-850/40">
            {analyticsDemoData.riskyIps.map((row) => {
              ...
              return (
                <tr key={row.id} className="hover:bg-slate-955/10 transition-colors">
                  <td className="py-3 font-mono text-[11.5px] font-bold text-slate-200 truncate select-all flex items-center space-x-2">
                    <Server className="w-3.5 h-3.5 text-slate-550 flex-shrink-0" />
                    <span>{row.ip}</span>
                  </td>
                  ...
                  <td className="py-3 text-center font-bold text-slate-400 font-sans">
                    {row.country}
                  </td>
                  <td className="py-3 text-right">
                    <div className="flex justify-end pr-2 text-red-450">
