...
export default function ThreatsByTypeChart() {
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-5 flex flex-col h-[340px] hover:border-slate-700 transition-colors justify-between text-left">
      {/* Header */}
      <div>
        <h2 className="text-sm font-bold text-white leading-none">Threats by Type</h2>
        <span className="text-[10px] text-slate-500 mt-1 block">Threat volume classifications</span>
      </div>
      ...
      <div className="flex-1 flex flex-col sm:flex-row items-center justify-between gap-4 min-h-0 mt-3">
        ...
        <div className="relative w-32 h-32 flex-shrink-0">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              ...
              />
              <Pie
                data={analyticsDemoData.threatsByType}
                cx="50%"
                cy="50%"
                innerRadius={42}
                outerRadius={58}
                ...
                dataKey="value"
              >
                {analyticsDemoData.threatsByType.map((entry, index) => (
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            <span className="text-sm font-extrabold text-white tracking-tight font-mono leading-none">
              12,845
            </span>
            <span className="text-[7.5px] text-slate-500 font-bold uppercase tracking-wider scale-90 mt-1">
              Total
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 w-full space-y-2 overflow-y-auto max-h-[190px] pr-1 scrollbar-thin scrollbar-thumb-slate-800">
          {analyticsDemoData.threatsByType.map((item) => (
